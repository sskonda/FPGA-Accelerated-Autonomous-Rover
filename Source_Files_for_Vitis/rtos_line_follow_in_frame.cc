 ...
 * Pure P control will follow the line, but it leaves a steady-state offset when
 * the robot has wheel mismatch or slips. This PI controller recenters the line
 * more aggressively and the low-pass filter keeps it from twitching.
 */
#define STEER_KP_NUM            1
#define STEER_KP_DEN            1
#define STEER_KI_NUM            1
#define STEER_KI_DEN            96
#define STEER_ERR_FILTER_NUM    1
#define STEER_ERR_FILTER_DEN    2
#define STEER_ERR_DEADBAND_PX   20
#define STEER_I_ACCUM_MIN       (-12000)
#define STEER_I_ACCUM_MAX       (12000)
#define STEER_DELTA_MAX         250
...
static bool vision_process_frame(int32_t *center_x,
...
#endif
                /* Camera is upside down, so small y is physically closer to the robot.
                 * Weight those rows more heavily so steering reacts to the part of the
                 * line the robot is about to drive over, not distant pixels. */
                const int32_t row_weight = ((ROI_Y_BOTTOM - y) / ROI_STRIDE_Y) + 1;
                weighted_sum_x += (int64_t)x * (int64_t)row_weight;
                weight_total   += (int64_t)row_weight;
                total_hits++;
            }
        }
    }
    ...
    *line_pixel_count = total_hits;
    ...
    int32_t err = 0;
    if ((total_hits >= MIN_LINE_PIXELS) && (weight_total > 0)) {
        cx    = (int32_t)(weighted_sum_x / weight_total);
...
for (;;) {
    ...
    if (s.sonar_ok && s.obstacle) {
         ...
         *
         * Low-pass filter reduces twitching from frame-to-frame noise.
         * Integral term removes steady offset caused by wheel mismatch/slip.
         */
        int32_t err_px = s.lane_error_px;
        if (i_abs32(err_px) <= STEER_ERR_DEADBAND_PX)
            err_px = 0;

        filt_err_px =
            ((filt_err_px * (STEER_ERR_FILTER_DEN - STEER_ERR_FILTER_NUM)) +
             (err_px      *  STEER_ERR_FILTER_NUM)) / STEER_ERR_FILTER_DEN;

        if (err_px == 0) {
            i_accum = (i_accum * 7) / 8;
        } else {
            i_accum += filt_err_px;
            i_accum = i_clamp(i_accum, STEER_I_ACCUM_MIN, STEER_I_ACCUM_MAX);
        }

        const int32_t delta_p = (filt_err_px * STEER_KP_NUM) / STEER_KP_DEN;
        const int32_t delta_i = (i_accum     * STEER_KI_NUM) / STEER_KI_DEN;
        int32_t delta = delta_p + delta_i;

        delta = i_clamp(delta, -STEER_DELTA_MAX, STEER_DELTA_MAX);
        ...
        /* No line: wait in place and clear controller memory so we do not
         * re-enter with a stale steering bias. */
        motor_all_brake();
        filt_err_px = 0;
        i_accum = 0;
